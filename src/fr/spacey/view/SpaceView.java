...
public class SpaceView implements Observer {
	...
	public void update(Observable obs, Object obj) {
		...
		for (Entity e : sc.getModel().getEntities()) {
			...
			if (e.isSimule()) {
				try {
					...
					LinkedList<Vector> trail = (LinkedList<Vector>) ((Simule) e).getTrail().clone();
					for (Vector v : trail) {
					...
					@SuppressWarnings("unchecked")
					LinkedList<Vector> next = (LinkedList<Vector>) ((Simule) e).getNext().clone();
					for (Vector v : next) {
		...
		gc.fillText("zoom: " + zoom + "  " + (width - width * zoom) / 2 + "   " + (height - height * zoom) / 2, relatX,
				110 - relatY);
		...
		if (sc.getModel().hasVaisseau()) {
			...
			gc.setFill(new LinearGradient(relatX + 158, height - 127 - relatY, relatX + 342, height - 108 - relatY,
					false, CycleMethod.NO_CYCLE, stops));
			gc.fillRoundRect(relatX + 158, height - 127 - relatY, sc.getModel().getVaisseau().getPropPrincipal() * 184,
					19, 15, 15);
			...
			double val = (sc.getModel().getVaisseau().getFuel() * 100 / sc.getModel().getVaisseau().getTankSize())
					/ 100;
